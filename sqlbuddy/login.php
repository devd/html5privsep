...
if (($adapter != "sqlite" && $host && $user && ($pass || $_POST)) || ($adapter == "sqlite" && $database)) {
	...
	if ($connCheck->isConnected()) {
		...
		$redirect = $protocol . $_SERVER["HTTP_HOST"] . $redirect . "/"."buddy.html";
...
<!-- Added for Ajax Form -->
    ...
    function submitAndRedirect(){
      var body=$('#loginForm1234').formSerialize();
      ...
      req.onreadystatechange = function (oEvent){
		...
		if(req.responseText.indexOf("Success")===0){
			window.top.location = 'buddy.html';
		}
	...
	<div id="container">
	<div id="loginform">
		<form  id='loginForm1234' name="loginform" method="post" action="javascript:submitAndRedirect()">
